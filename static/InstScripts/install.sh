...
install_portainer() {
    ...
    # Pre-create admin at startup; Portainer does not require a setup token.
    # https://docs.portainer.io/faqs/installing/setup-token
    local password_file="$(pwd)/portainer-admin-password.txt"
    if [[ ! -f "$password_file" ]]; then
        local password
        password=$(generate_secret 18) || return 1
        (umask 077; printf '%s' "Aa1!${password}" > "$password_file") || return 1
    fi
    [[ -s "$password_file" ]] || { error "Portainer password file is empty."; return 1; }
    ...
    docker run -d \
        ...
        -v "$password_file:/run/secrets/portainer-admin-password:ro" \
        portainer/portainer-ce:latest \
        --admin-password-file /run/secrets/portainer-admin-password || return 1

    ok "Portainer installed."
    info "New installation: sign in as admin with the password in $password_file."
    info "Existing Portainer accounts retain their current passwords."
...
COMPOSE_SHA256="d3923a4921c2759bf365d17df1cd9744cd4e6fd70242e324fea31e90b815f96f"
NGINX_SHA256="e0324656d6e0e24c87639b7ca70c8128f235f1d3f463355ece472beec6c432eb"
...
create_shared_network() {
    local network
    for network in internetwork external; do
        if ! docker network inspect "$network" >/dev/null 2>&1; then
            docker network create "$network" >/dev/null || return 1
        fi
        ok "Shared network '$network' is ready."
    done
}

configure_nginx() {
    local hostname hosts temporary
    hostname=$(sed -n 's/^NGINX_SERVER_NAME=//p' .env | head -n 1 | tr -d '\r')
    if [[ -z "$hostname" ]]; then
        # Support .env files produced by older installers without rotating secrets.
        hosts=$(sed -n 's/^ALLOWED_HOSTS=//p' .env | head -n 1 | tr -d '\r')
        hostname=$(printf '%s' "$hosts" | tr ',' '\n' | awk '!/^(localhost|127\.0\.0\.1|backend|nginx)$/ && NF {print; exit}')
        hostname="${hostname:-localhost}"
    fi
    [[ "$hostname" =~ ^[a-zA-Z0-9][a-zA-Z0-9.-]*$ ]] || {
        error "NGINX_SERVER_NAME must be a hostname or IPv4 address without a scheme, port or path."
        return 1
    }
    temporary=$(mktemp ./nginx.conf.XXXXXX) || return 1
    sed "s/server_name localhost;.*/server_name ${hostname};/" nginx.conf > "$temporary" || return 1
    mv "$temporary" nginx.conf
    ok "nginx server_name set to $hostname."
}
...
setup_project() {
    ...
    create_shared_network || return 1
...
install_portainer || { error "Portainer setup failed."; exit 1; }
